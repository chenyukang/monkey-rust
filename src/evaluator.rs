use crate::ast::*;
use crate::object;
use crate::object::{Array, Builtin, Environment, Function, MonkeyHash, Object};
...
mod test {
    ...
    fn error_handling() {
        ...
        let tests = vec![
            Test {
                input: "5 + true;",
                expected: "type mismatch: Int(5) + Bool(true)",
            },
            Test {
                input: "5 + true; 5;",
                expected: "type mismatch: Int(5) + Bool(true)",
            ...
            Test {
                input: "-true",
                expected: "unknown operator: -Bool(true)",
            },
            Test {
                input: "true + false",
            ...
            Test {
                ...
                expected: "unusable as hash key: fn(x) {\nx\n}",
            },
        ];
...
addTwo(2);";
    ...
    fn builtin_functions() {
        ...
        for t in tests {
            ...
            match (&t.expected, &*obj) {
                (Object::Int(exp), Object::Int(got)) => assert_eq!(
                    *exp, *got,
    ...
    fn builtin_errors() {
        ...
        let tests = vec![
            Test {
                input: r#"len(1)"#,
                expected: "object Int(1) not supported as an argument for len",
            },
            Test {
                input: r#"len("one", "two")"#,
                expected: "len takes only 1 array or string argument",
            },
            Test {
                input: r#"first(1)"#,
                expected: "object Int(1) not supported as an argument for first",
            },
        ];
        ...
        for t in tests {
            ...
            match parser::parse(t.input) {
                Ok(node) => match eval(&node, env) {
                    ...
                    Err(err) => assert_eq!(
                        t.expected, err.message,
                        "on input {} expected error {} but got {}",
                        t.input, t.expected, err.message
                    ),
                },
                Err(e) => panic!("error {:?} on input {}", e, t.input),
            }
            ...
            Object::Hash(h) => {
                ...
                for (key, value) in &h.pairs {
                    match (&*Rc::clone(key), &*Rc::clone(value)) {
                        (Object::String(k), Object::Int(val)) => match k.as_str() {
                            "one" => assert_eq!(*val, 1),
                            "two" => assert_eq!(*val, 2),
